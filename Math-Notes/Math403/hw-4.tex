...
\noindent
\textsc{Reading assignments}\total{}{48}%covers Lectures 16 - 22
...
can't strecth more than the product of the individual matrices. If a more 
rigorous proof is needed, assume that it is false, and that there is 
a vector $x$ such that $|ABx|> ||A||_{2}||B||_{2}x$. But the $|Bx|$ is bounded by
...
are only 1's in the superdiagonal. Now change the basis of so that the ones become arbigtrarily small, so $x$ can't be arbitrarily close to the 
...
is also in $SU_{n}$. So $e^{tA}$ is both in $U_n$ and $SL_n$.
